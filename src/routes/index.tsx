...
function HomePage() {
  ...
  // WhatsApp number — from DB if available, fallback hardcoded
  const rawWa = settings?.whatsappNumber ?? settings?.contactPhone ?? "";
  const waNum = rawWa.replace(/[^0-9]/g, "").replace(/^0/, "92") || "923000000000";
  const waLink = `https://wa.me/${waNum}?text=${encodeURIComponent("Hi Timera! I need help choosing a watch.")}`;

  return (
    <>
      {/* WhatsApp floating button — always visible on homepage */}
      <a
        href={waLink}
        target="_blank"
        rel="noopener noreferrer"
