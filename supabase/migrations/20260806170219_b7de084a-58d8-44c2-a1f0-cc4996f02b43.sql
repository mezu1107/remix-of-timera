...
WITH CHECK (
  ...
  AND (currency = ANY (ARRAY['PKR'::text,'USD'::text]))
  AND (value IS NULL OR value >= 0)
  AND (user_agent IS NULL OR length(user_agent) <= 512)
  AND pg_column_size(metadata) <= 4096
);
UPDATE public.site_settings SET whatsapp_number = '923108158565' WHERE whatsapp_number IS NULL OR whatsapp_number = '';